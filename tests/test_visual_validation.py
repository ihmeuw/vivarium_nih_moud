...
def test_add_age_group():
    ...
    pop = pd.DataFrame({"age": [2, 7, 12, 17, 99, 101]})
...
def test_validation_plots_structure(mock_subplots, mock_show, mock_series_plot):
    ...
    pop = pd.DataFrame(
        {
            ...
            cause: np.random.choice(
                [f"susceptible_to_{cause}", f"infected_with_{cause}"], pop_size
            ),
    ...
    for sex in ["Male", "Female"]:
        for age in ages:
            data.append({"sex": sex, "age_start": age, "age_end": age + 5, "value": 0.01})
