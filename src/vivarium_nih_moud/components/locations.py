...
class DiseaseRisk(Component):
    ...
    def setup(self, builder: Builder) -> None:
        ...
        self.disease_model = builder.components.get_component(
            f"disease_model.{self.cause.name}"
        )
        ...
        for state_name in self.state_mapping.values():
            if state_name not in [state.state_id for state in self.disease_model.states]:
                raise ValueError(
                    f"State {state_name} not found in disease model {self.cause.name}"
                )
...
def quarters_risk():
    ...
    return DiseaseRisk(cause="quarters", cat1="unhoused", cat2="incarcerated", cat3="housed")
...
class SimpleRiskEffect(Component):
    ...
    def __init__(self, risk_name: str, affected_pipeline_name: str):
        ...
        self.config_name = (
            f"effect_of_{self.risk.name}_on_{self.affected_pipeline_name}".replace(
                ".transition_rate", ""
            )
        )
    ...
    def setup(self, builder: Builder) -> None:
        ...
        self.relative_risk_mapping = builder.configuration[self.config_name][
            "relative_risk"
        ].to_dict()

        # Get the risk exposure pipeline
        self.risk_exposure_pipeline = builder.value.get_value(f"{self.risk.name}.exposure")
    ...
    def calculate_paf(self, s_relative_risk: pd.Series) -> None:
        ...
        # breakpoint()
        assert (
            not s_relative_risk.isnull().any()
        ), f"Relative risk series contains NaN values, check config {self.config_name}.relative_risk for missing categories."
        ...
        print(
            f"Calculated PAF for {self.risk.name} effect on {self.affected_pipeline_name}: {self.paf_value}"
        )
