...
Individual-based microsimulation of opioid use disorder calibrated to Global Burden of Disease estimates.
...
all necessary requirements as follows::
  ...
  :~$ conda create --name=vivarium_nih_moud python=3.11
  ...
  (vivarium_nih_moud) :~$ cd vivarium_nih_moud
...
`pip install -e .` with the following::
  ...
  (vivarium_nih_moud) :~$ pip install -r requirements.txt
...
install hdf tooling from within your environment like so::
  ...
  (vivarium_nih_moud) :~$ conda install hdf5

The ``(vivarium_nih_moud)`` that precedes your shell prompt will probably show
...
``src/vivarium_nih_moud`` package directory:
...
- ``components``
  ...
  the vivarium_nih_moud project. You should work with the
